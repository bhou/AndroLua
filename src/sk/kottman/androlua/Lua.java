...
public class Lua extends Service {
	...
	// currently this is just so that the main activity knows when the service
	// is up...
	...
	public boolean createLuaThread(String mod, Object arg, Object progress,
			Object post) {
		if (progress != null && !(progress instanceof LuaObject))
			return false;
		if (post != null && !(post instanceof LuaObject))
			return false;
		new LuaThread((LuaObject) progress, (LuaObject) post, false).execute(
				mod, arg);
		return true;
	}
	...
	public String evalLua(String src, String chunkName) throws LuaException {
		...
		throw new LuaException(LuaObject.errorReason(ok) + ": "
				+ L.toString(-1));
	}
		...
		public void run() {
			...
			try {
				...
				while (!stopped) {
					...
					} else if (line.equals("combine")) { // _all_ output goes to
															// 3333!
						printer = out;
						writer = null;
					} else {
